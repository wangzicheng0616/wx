	...
	<view class="u-wrap" :style="{
	 ...
	 :class="'u-lazy-item-' + elIndex">
		<view :class="'u-lazy-item-' + elIndex">
			<image :style="{borderRadius: borderRadius + 'rpx', height: imageHeight}" v-if="!isError" class="u-lazy-item"
			 :src="isShow ? image : loadingimage" :mode="imageMode" @load="imageLoaded" @error="loadError" @tap="clickimage"></image>
			<image :style="{borderRadius: borderRadius + 'rpx', height: imageHeight}" class="u-lazy-item error" v-else :src="errorimage"
			 :mode="imageMode" @load="errorimageLoaded" @tap="clickimage"></image>
		</view>
	</view>
</template>
...
<script>
	 ...
	 * @property {String} loading-image 预加载时的占位图
	 * @property {String} error-image 图片加载出错时的占位图
	 ...
	 * @property {String Number} height 图片高度，注意：实际高度可能受image-mode参数影响（默认450）
	 * @property {String Number} mg-mode 图片的裁剪模式，详见image组件裁剪模式（默认widthFix）
	 * @event {Function} click 点击图片时触发
	 * @event {Function} load 图片加载成功时触发
	 * @event {Function} error 图片加载失败时触发
	 * @example <u-lazy-load :image="image" :loading-image="loadingimage" :error-image="errorimage"></u-lazy-load>
	 */
	export default {
		name: 'u-lazy-load',
		props: {
		...
		methods: {
			...
			clickimage() {
				...
				if (this.isShow == false) whichimage = 'lazyimage';
				// 如果isError为true，意味着图片加载失败，这是只剩下错误的占位图，所以点击的只能是错误占位图
				// 当然，也可以给错误的占位图元素绑定点击事件，看你喜欢~
				else if (this.isError == true) whichimage = 'errorimage';
				// 总共三张图片，除了两个占位图，剩下的只能是正常的那张图片了
				else whichimage = 'realimage';
				// 只通知当前图片的index
