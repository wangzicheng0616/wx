<template>
	<view>
		...
		<view id="_top" :style="showAm+(selectable?';user-select:text;-webkit-user-select:text':'')">
			...
			<!--#endif-->
			<!--#ifndef H5 || MP-360-->
			<trees :nodes="nodes" :lazyLoad="lazyLoad" :loading="loadingimage" />
			<!--#endif-->
		</view>
		<!--#endif-->
	</view>
...
<script>
	 ...
	 * @property {String} loadingimage 图片加载完成前的占位图
	 ...
	 * @event {Function} imagetap 图片点击事件
	...
	export default {
		...
		created() {
			// 图片数组
			this.imageList = [];
			this.imageList.each = function(f) {
				for (var i = 0, len = this.length; i < len; i++)
					this.setItem(i, f(this[i], i, this));
			}
			this.imageList.setItem = function(i, src) {
		...
		methods: {
			...
			setContent(html, append) {
				...
				if (append)
					...
					html =
						...
						'</view><script>"use strict";function e(e){if(window.__dcloud_weex_postMessage||window.__dcloud_weex_){var t={data:[e]};window.__dcloud_weex_postMessage?window.__dcloud_weex_postMessage(t):window.__dcloud_weex_.postMessage(JSON.stringify(t))}}document.body.onclick=function(){e({action:"click"})},' +
				...
				this.$refs.web.evalJs(
					'var t=document.getElementsByTagName("title");t.length&&e({action:"getTitle",title:t[0].innerText});for(var o,n=document.getElementsByTagName("style"),r=1;o=n[r++];)o.innerHTML=o.innerHTML.replace(/body/g,"#parser");for(var a,c=document.getElementsByTagName("image"),s=[],i=0==c.length,d=0,l=0,g=0;a=c[l];l++)parseInt(a.style.width||a.getAttribute("width"))>' +
					windowWidth + '&&(a.style.height="auto"),a.onload=function(){++d==c.length&&(i=!0)},a.onerror=function(){++d==c.length&&(i=!0),' + (cfg.errorimage ? 'this.src="' + cfg.errorimage + '",' : '') +
					'e({action:"error",source:"image",target:this})},a.hasAttribute("ignore")||"A"==a.parentElement.nodeName||(a.i=g++,s.push(a.getAttribute("original-src")||a.src||a.getAttribute("data-src")),a.onclick=function(t){t.stopPropagation(),e({action:"preview",image:{i:this.i,src:this.src}})});e({action:"getimageList",imageList:s});for(var u,m=document.getElementsByTagName("a"),f=0;u=m[f];f++)u.onclick=function(m){m.stopPropagation();var t,o=this.getAttribute("href");if("#"==o[0]){var n=document.getElementById(o.substr(1));n&&(t=n.offsetTop)}return e({action:"linkpress",href:o,offset:t}),!1};for(var h,y=document.getElementsByTagName("video"),v=0;h=y[v];v++)h.style.maxWidth="100%",h.onerror=function(){e({action:"error",source:"video",target:this})}' +
					(this.autopause ? ',h.onplay=function(){for(var e,t=0;e=y[t];t++)e!=this&&e.pause()}' : '') +
					';for(var _,p=document.getElementsByTagName("audio"),w=0;_=p[w];w++)_.onerror=function(){e({action:"error",source:"audio",target:this})};' +
					(this.autoscroll ? 'for(var T,E=document.getElementsByTagName("table"),B=0;T=E[B];B++){var N=document.createElement("view");N.style.overflow="scroll",T.parentNode.replaceChild(N,T),N.appendChild(T)}' : '') +
				...
				var images = this.rtf.getElementsByTagName('image');
				for (let i = 0, j = 0, image; image = images[i]; i++) {
					if (parseInt(image.style.width || image.getAttribute('width')) > windowWidth)
						image.style.height = 'auto';
					fill(image);
					if (!image.hasAttribute('ignore') && image.parentElement.nodeName != 'A') {
						image.i = j++;
						_ts.imageList.push(image.getAttribute('original-src') || image.src || image.getAttribute('data-src'));
						image.onclick = function(e) {
							e.stopPropagation();
							var preview = true;
							this.ignore = () => preview = false;
							_ts.$emit('imagetap', this);
							if (preview) {
								uni.previewImage({
									current: this.i,
									urls: _ts.imageList
					...
					image.onerror = function() {
						if (cfg.errorimage)
							_ts.imageList[this.i] = this.src = cfg.errorimage;
						_ts.$emit('error', {
							source: 'image',
							target: this
						});
					}
					if (_ts.lazyLoad && this._observer && image.src && image.i != 0) {
			...
			_handleHtml(html, append) {
				if (!append) {
					...
					var style = '<style>@keyframes _show{0%{opacity:0}100%{opacity:1}}image{max-width:100%}';
			...
			_message(e) {
				...
				switch (d.action) {
					...
					case 'getimageList':
						this.imageList.length = 0;
						for (var i = d.imageList.length; i--;)
							this.imageList.setItem(i, d.imageList[i]);
						break;
					case 'preview':
						var preview = true;
						d.image.ignore = () => preview = false;
						this.$emit('imagetap', d.image);
						if (preview)
							uni.previewImage({
								current: d.image.i,
								urls: this.imageList
					...
					case 'linkpress':
						...
						break;
					case 'error':
						if (d.source == 'image' && cfg.errorimage)
							this.imageList.setItem(d.target.i, cfg.errorimage);
						this.$emit('error', {
							source: d.source,
							target: d.target
						})
