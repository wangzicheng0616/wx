<template>
	<view :class="'interlayer '+(c||'')" :style="s">
		<block v-for="(n, i) in nodes" v-bind:key="i">
			<!--图片-->
			<view v-if="n.name=='image'" :class="'_image '+n.attrs.class" :style="n.attrs.style" :data-attrs="n.attrs" @tap.stop="imagetap">
				<rich-text v-if="ctrl[i]!=0" :nodes="[{attrs:{src:loading&&(ctrl[i]||0)<2?loading:(lazyLoad&&!ctrl[i]?placeholder:(ctrl[i]==3?errorimage:n.attrs.src||'')),alt:n.attrs.alt||'',width:n.attrs.width||'',style:'-webkit-touch-callout:none;max-width:100%;display:block'+(n.attrs.height?';height:'+n.attrs.height:'')},name:'image'}]" />
				<image class="_image" :src="lazyLoad&&!ctrl[i]?placeholder:n.attrs.src" :lazy-load="lazyLoad"
				 :show-menu-by-longpress="!n.attrs.ignore" :data-i="i" :data-index="n.attrs.i" data-source="image" @load="loadimage"
				 @error="error" />
			</view>
			<!--文本-->
...
<script>
	...
	const errorimage = require('../libs/config.js').errorimage;
	export default {
		components: {
			trees
		},
		...
		methods: {
			init() {
				for (var i = this.nodes.length, n; n = this.nodes[--i];) {
					if (n.name == 'image') {
						this.top.imageList.setItem(n.attrs.i, n.attrs['original-src'] || n.attrs.src);
						// #ifdef APP-PLUS
						if (this.lazyLoad && !this.observer) {
							...
							setTimeout(() => {
								this.observer.observe('._image', res => {
									if (res.intersectionRatio) {
										for (var j = this.nodes.length; j--;)
											if (this.nodes[j].name == 'image')
			...
			imagetap(e) {
				...
				if (!attrs.ignore) {
					...
					global.Parser.onimagetap && global.Parser.onimagetap(data);
					this.top.$emit('imagetap', data);
					if (preview) {
						var urls = this.top.imageList,
			...
			error(e) {
				...
				} else if (errorimage && source == 'image') {
					this.top.imageList.setItem(target.dataset.index, errorimage);
